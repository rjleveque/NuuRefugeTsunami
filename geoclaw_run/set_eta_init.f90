...
subroutine set_eta_init(mbc,mx,my,xlow,ylow,dx,dy,t,veta)
    ...
    if (.true.) then
        ...
        ! Set for the Nu'u Pond:
        lake_level = 1.0d0  ! meters relative to 0 datum of topofiles
        x1_lake = -156.178d0
        x2_lake = -156.176d0
        y1_lake = 20.6265d0
        y2_lake = 20.6280d0
    ...
    ! for Nu'u pond case we do not want to adjust eta for dtopo, 
    ! which is nonzero at time 0 for the submarine slump, so quit here:
    return
